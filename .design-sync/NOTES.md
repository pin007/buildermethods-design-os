...
## WCAG remediation (2026-07-11)
- aria-label 159 in source & compiled bundle; htmlFor 41; ToggleSwitch API now `{enabled, onChange, label (required), size?, disabled?}` with role=switch — StrategyCard/StrategyDetail import it from `@/sections/settings-and-operations/components/SettingsDetailLayout` (NOT the barrel — the converter's alias plugin can't resolve directory imports; keep concrete-module imports for cross-section deps).
- Token bumps: light `--faint` #79716B; dark `--hint` #84848A, `--faint` #7E7E84 (all ≥4.5:1 vs background AND card; light `--hint` untouched).
- Semantic-token migration: zinc+pink 7120 → 1421 (80%); the remainder are deliberate keeps (chart marks/axes, translucent overlays, dark-only accents, danger surfaces, state scales, hover-emphasis pairs with no token). Financial state colors stay raw by convention.
- build.sh annotates all `--tw-*` internals with `/* @kind other */` (558) so the DS token check skips Tailwind internals.
- Known keyboard gaps (structural, deferred): clickable `<tr onClick>` rows (JournalEntries, GuruTracker alerts, ResearchSchedule history, PortfolioOverview, StrategyDetail, OrdersScreen, Backtest/WFO tables) and `<th onClick>` sort headers (SourceDetail, DataQuality, CorporateActions, Backtest/WFO) have no keyboard path; PositionsTab/OrdersScreen use real button headers — pattern to adopt. Nested-interactive chips in SentimentTab. Pre-existing bugs: `onBack` not destructured (SourceDetail, StrategyDetail), invalid `dark:hover:bg-zinc-750` (BacktestResults).
