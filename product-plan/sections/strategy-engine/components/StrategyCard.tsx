  ...
  return (
    <div
      className={`
        ...
        ${isActive
          ? 'border-l-[3px] border-l-pink-600 border-t-stone-200 border-r-stone-200 border-b-stone-200 dark:border-l-pink-500 dark:border-t-zinc-800 dark:border-r-zinc-800 dark:border-b-zinc-800 bg-white dark:bg-zinc-900'
          : 'border-stone-200 dark:border-zinc-800 bg-stone-50/50 dark:bg-zinc-900/50 opacity-75'
        }
        hover:shadow-md dark:hover:shadow-zinc-950/40 hover:border-stone-300 dark:hover:border-zinc-700
      `}
      onClick={onView}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="flex items-start justify-between p-5 pb-3">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="text-base font-semibold text-stone-900 dark:text-zinc-100 truncate pr-8">
            {strategy.name}
          </h3>
          <p className="mt-0.5 text-xs text-stone-500 dark:text-zinc-500 line-clamp-1">
      ...
      <div className="px-5 pb-3">
        <div className="flex flex-wrap gap-1.5">
          {strategy.instruments.slice(0, 5).map((inst) => (
            <span
              key={inst}
              className="inline-flex items-center rounded-md bg-stone-100 dark:bg-zinc-800 px-2 py-0.5 text-xs font-medium text-stone-600 dark:text-zinc-400"
            >
              {inst}
            </span>
          ))}
          {strategy.instruments.length > 5 && (
            <span className="inline-flex items-center rounded-md bg-stone-100 dark:bg-zinc-800 px-2 py-0.5 text-xs font-medium text-stone-400 dark:text-zinc-500">
              +{strategy.instruments.length - 5}
            </span>
          )}
        </div>
      ...
      {/* Metrics */}
      {bt ? (
        <div className="mx-5 rounded-xl bg-stone-50 dark:bg-zinc-800/50 p-3.5">
          <div className="grid grid-cols-3 gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400 dark:text-zinc-500 mb-0.5">
                Sharpe
              </p>
        ...
        <div className="mx-5 rounded-xl bg-stone-50 dark:bg-zinc-800/50 p-3.5">
          <p className="text-xs text-stone-400 dark:text-zinc-500 italic text-center">
            No backtests yet
          </p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-5 py-3 mt-1">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-lg p-1.5 text-stone-300 hover:text-red-500 dark:text-zinc-600 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all duration-150 hover:bg-red-50 dark:hover:bg-red-950/30"
            aria-label="Delete strategy"
          >
            <Trash2 size={14} strokeWidth={1.5} />
          </button>
          <div className="text-stone-300 dark:text-zinc-600 opacity-0 group-hover:opacity-100 transition-opacity">
